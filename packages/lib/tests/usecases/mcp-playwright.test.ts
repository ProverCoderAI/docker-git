  ...
  it.effect("enables Playwright MCP for an existing project without rewriting env files", () =>
    withTempDir((root) =>
      Effect.gen(function*(_) {
        ...
        expect(cdpGuardExists).toBe(true)
        ...
        const cdpGuard = yield* _(fs.readFileString(path.join(outDir, "docker-git-cdp-guard")))
        ...
        expect(browserDockerfile).toContain("COPY docker-git-cdp-guard /usr/local/bin/docker-git-cdp-guard")
        expect(browserDockerfile).not.toContain("RUN cat <<'EOF' > /usr/local/bin/docker-git-cdp-guard")
        expect(browserDockerfile).toContain("ws@8.18.3")
        expect(cdpGuard).toContain("#!/usr/bin/env node")
        expect(cdpGuard).toContain("WebSocketServer")
        expect(cdpGuard).toContain('const upstreamHost = "127.0.0.1";')
        expect(cdpGuard).toContain("const upstreamPort = 9222;")
        expect(cdpGuard).toContain('const listenHost = "0.0.0.0";')
        expect(cdpGuard).toContain("const listenPort = 9223;")
        expect(cdpGuard).not.toContain("MCP_PLAYWRIGHT_UPSTREAM_CDP_HOST")
        expect(cdpGuard).not.toContain("MCP_PLAYWRIGHT_CDP_GUARD_PORT")
        expect(cdpGuard).toContain("Browser.close")
        expect(cdpGuard).toContain("Browser.crash")
        expect(startExtra).toContain('MCP_PLAYWRIGHT_CDP_GUARD:-1')
        expect(startExtra).toContain("docker-git-cdp-guard")
        expect(startExtra).toContain('guard_pid="$!"')
        expect(startExtra).toContain("falling back to socat")
        expect(startExtra).toContain("socat TCP-LISTEN:9223,fork,reuseaddr TCP:127.0.0.1:9222")
        ...
        expect(browserRuntime).toContain('printf \'%s\\n\' "http://127.0.0.1:9223"')
        expect(browserRuntime).not.toContain('printf \'%s\\n\' "${MCP_PLAYWRIGHT_CDP_ENDPOINT:-http://127.0.0.1:9223}"')
