// CHANGE: extract the Gemini Android MCP config sync into its own module
// WHY: issue-436 wires mcp-android "the same way" Playwright MCP works; keeping the
//      render helper in a dedicated file keeps gemini.ts under the max-lines lint budget
// QUOTE(ТЗ): "Подключить mcp-android так же как работает MCP PLAYRIGHT"
// REF: issue-436
// SOURCE: n/a
export const renderGeminiMcpAndroidConfig = (): string =>
...
docker_git_sync_gemini_android_mcp() {
  ...
  GEMINI_CONFIG_SETTINGS_FILE="$GEMINI_CONFIG_SETTINGS_FILE" MCP_ANDROID_ENABLE="${"$"}{MCP_ANDROID_ENABLE:-0}" DOCKER_GIT_ANDROID_PROJECT="$android_project" DOCKER_GIT_ANDROID_NETWORK="$android_network" DOCKER_GIT_ANDROID_ADB_ENDPOINT="$adb_endpoint" TARGET_DIR="${"$"}{TARGET_DIR:-}" node - <<'NODE'
