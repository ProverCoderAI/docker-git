 ...
 * @returns Effect with a typed session upload context, or DecodeError when required fields are invalid.
 * @pure false - boundary decoder for persisted JSON.
 * @effect Effect<SessionUploadContext, DecodeError, never>
...
// PURITY: SHELL
// EFFECT: Effect<SessionUploadContext, DecodeError, never>
// INVARIANT: invalid or incomplete context fails with DecodeError.
// COMPLEXITY: O(n)/O(n), where n is encoded field count.
export const decodeSessionUploadContext = (value: unknown): Effect.Effect<SessionUploadContext, DecodeError> =>
  decodeUnknownEffect(SessionUploadContextSchema, "session upload context", value)

/**
 * Decodes the child-process readiness handshake payload.
 *
 * @param value - Unknown JSON value read from the readiness file.
 * @returns Effect with a typed readiness state, or DecodeError when the payload violates the schema.
 * @pure false - boundary decoder for JSON produced by another process.
 * @effect Effect<BackgroundReadyState, DecodeError, never>
 * @invariant decode(valid started|failed state) succeeds; decode(invalid) fails with DecodeError.
 * @precondition value may be any JSON-compatible value.
 ...
 */
export const decodeBackgroundReadyState = (value: unknown): Effect.Effect<BackgroundReadyState, DecodeError> =>
  decodeUnknownEffect(BackgroundReadyStateSchema, "background ready state", value)
 ...
 * @returns Effect with normalized repository info, or DecodeError when the response shape is invalid.
 * @pure false - boundary decoder for GitHub API JSON.
 * @effect Effect<GitHubRepoInfo, DecodeError, never>
 * @invariant missing nullable fields normalize to null; invalid objects fail with DecodeError.
 * @precondition value may be any GitHub API JSON response.
 * @postcondition defaultBranch and htmlUrl are either strings or null.
 * @complexity O(n) time / O(n) space where n is the decoded field count.
 */
export const decodeGitHubRepoInfo = (value: unknown): Effect.Effect<GitHubRepoInfo, DecodeError> =>
  decodeUnknownEffect(GitHubRepoInfoSchema, "GitHub repository response", value).pipe(
    Effect.map((repo) => ({
 ...
 * @returns Effect with a typed PR comment reference, or DecodeError when the response shape is invalid.
 * @pure false - boundary decoder for GitHub API JSON.
 * @effect Effect<PrComment, DecodeError, never>
 ...
 * @complexity O(n) time / O(n) space where n is the decoded field count.
 */
export const decodeGitHubPrComment = (value: unknown): Effect.Effect<PrComment, DecodeError> =>
  decodeUnknownEffect(GitHubPrCommentResponseSchema, "GitHub PR comment response", value).pipe(
    Effect.map((comment) => ({
      id: comment.id,
 ...
 * @returns Effect with a base64 content response, or DecodeError when encoding/content are invalid.
 * @pure false - boundary decoder for GitHub API JSON.
 * @effect Effect<GitHubContentResponse, DecodeError, never>
 * @invariant decode(value) succeeds -> value.encoding = "base64".
 * @precondition value may be any GitHub API JSON response.
 * @postcondition returned content is a string and may be empty for empty files.
 * @complexity O(n) time / O(n) space where n is the decoded field count.
 */
export const decodeGitHubContentResponse = (value: unknown): Effect.Effect<GitHubContentResponse, DecodeError> =>
  decodeUnknownEffect(GitHubContentResponseSchema, "GitHub content response", value)
 ...
 * @returns Effect with a 40-character hexadecimal Git object SHA.
 * @pure false - boundary decoder for GitHub API JSON.
 * @effect Effect<string, DecodeError, never>
 * @invariant returned sha matches /^[0-9a-f]{40}$/i.
 * @precondition value may be any GitHub API JSON response.
 * @postcondition callers never receive null or malformed SHAs.
 * @complexity O(n) time / O(n) space where n is the decoded field count.
 */
export const decodeGitHubSha = (
  value: unknown,
  context: string = "GitHub response"
): Effect.Effect<string, DecodeError> =>
  decodeUnknownEffect(GitHubShaResponseSchema, context, value).pipe(
    Effect.map((response) => response.sha)
  )
 ...
 * @returns Effect with a readonly collection of tree entries, or DecodeError when the response is invalid.
 * @pure false - boundary decoder for GitHub API JSON.
 * @effect Effect<ReadonlyArray<TreeEntry>, DecodeError, never>
 * @invariant invalid tree payloads fail with DecodeError, never silently decode to [].
 * @precondition value may be any GitHub API JSON response.
 ...
 */
export const decodeGitHubTreeEntries = (value: unknown): Effect.Effect<ReadonlyArray<TreeEntry>, DecodeError> =>
  decodeUnknownEffect(GitHubTreeResponseSchema, "GitHub tree response", value).pipe(
    Effect.map((response) => response.tree)
  )
