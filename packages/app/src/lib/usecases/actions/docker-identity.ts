  ...
  Effect.gen(function*(_) {
    ...
    for (const configPath of configPaths) {
      const status = yield* _(
        loadProjectStatus(configPath).pipe(Effect.match({ onFailure: () => null, onSuccess: (v) => v }))
      )
...
export const deleteConflictingProjectsIfNeeded = (
  ...
  Effect.gen(function*(_) {
    ...
    for (const conflictingProject of projects.values()) {
      yield* _(
        Effect.logWarning(`Force enabled: replacing conflicting docker-git project ${conflictingProject.projectDir}`)
      )
